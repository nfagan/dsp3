function save_lfp(source_pl2_files, conf, overwrite)
...
save_p = fullfile( dsp3.dataroot(conf), 'raw', 'lfp' );
shared_utils.io.require_dir( save_p );

parfor i = 1:numel(source_pl2_files)
  shared_utils.general.progress( i, numel(source_pl2_files) );
  
  [one_file_outs, filename] = one_file( source_pl2_files{i}, consolidated );
  
  save_filepath = fullfile( save_p, filename );
  
  if ( shared_utils.io.fexists(save_filepath) && ~overwrite )
    fprintf( '\n Skipping "%s" because it already exists.', save_filepath );
  else
    shared_utils.io.psave( save_filepath, one_file_outs );
  end
end

end

function [out, save_filename] = one_file(pl2_file, consolidated)

pl2_filename = shared_utils.io.filenames( pl2_file, true );
save_filename = sprintf( '%s.mat', shared_utils.io.filenames(pl2_file) );
